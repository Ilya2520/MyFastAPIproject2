...
from database import MenuModel, SubmenuModel, DishModel, Session
...
def test_menu_update():
    ...
    response = client.post(url, json=menu)
    id = response.json()["id"]
    updated_menu = {"title": "update menu1", "description": "update menu1 description"}
    update_url = f"{url}{id}"
...
def test_menu_update_error():  # отправляем запрос на обновление несуществующего меню
    clear_database()
    url = "api/v1/menus/"
    menu = {"title": "menu1", "description": "menu1 description"}
    response = client.post(url, json=menu)
...
def test_menu_delete_error():  # отправляем запрос на удаление несуществующего меню
    clear_database()
    url = "api/v1/menus/"
    menu = {"title": "menu1", "description": "menu1 description"}
    response = client.post(url, json=menu)
...
def test_submenu_update():
    ...
    response = client.post(url, json=submenu)
    id = response.json()["id"]
    submenu = {"title": "update submenu1", "description": "update submenu1 description"}
    url = f"{url}/{id}"
...
def test_dish_post():
    ...
    response = client.post(url, json=submenu)
    id = response.json()["id"]
    dish = {"title": "dish", "description": "dish description", "price": "12.50"}
    url = f"{url}/{id}/dishes"
...
def test_dish_update():
    ...
    response = client.post(url, json=dish)
    id = response.json()["id"]
    dish2 = {"title": "update dish", "description": "update dish description", "price": "1"}
    url = f"{url}/{id}"
...
def dish_create(menu_id: uuid.UUID, submenu_id: uuid.UUID, dish: str):
    ...
    new_dish = {"title": dish, "description": f"{dish} description", "price": "11.5"}
...
def test_count_of_dishes():
    ...
    dish1_id = dish_create(menu_id, submenu_id, "dish1")
    dish2_id = dish_create(menu_id, submenu_id, "dish2")
    menu_get(menu_id)
